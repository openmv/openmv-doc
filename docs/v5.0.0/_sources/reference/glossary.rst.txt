...
.. glossary::
    ...
    bytecode
        A compact representation of a Python program that is generated by
    ...
    callee-owned tuple
        ...
        a new tuple for every call, and reduces :term:`heap` fragmentation. Programs
    ...
    frozen module
        A Python module that has been :term:`cross compiled <cross-compiler>`
        and bundled into the firmware image. This reduces RAM requirements as
    ...
    MCU
        Microcontroller. Microcontrollers usually have far fewer resources
    ...
    micropython-lib
        ...
        file with just a few builtin modules. There is no extensive standard
        ...
        Some of the modules are implemented in pure Python and run on any
        port; the OpenMV firmware freezes a selection of these (sensor
        drivers, networking, and utility helpers) directly into the
        firmware image so they can be imported without installing
        anything. A smaller number of micropython-lib modules use
        :term:`FFI` and only work on the :term:`MicroPython Unix port`;
        those are not available on OpenMV Cams.

        Unlike the :term:`CPython` stdlib, micropython-lib modules that
        are not frozen in are installed individually - either by copying
        them onto the filesystem or, on network-capable boards, using
        :term:`mip`.
    ...
    native
        ...
        microcontroller (ARM Thumb on the Cortex-M based OpenMV Cams). The
        ``@micropython.native`` decorator can be applied to a MicroPython
        function to generate native code instead of :term:`bytecode` for
        that function, which will likely be faster but use more RAM.
    ...
    pyboard.py
        A now-superseded host-side tool for interacting with a MicroPython
        board — running scripts and accessing the filesystem over the raw
        :term:`REPL`. See its replacement, :term:`mpremote`.
    ...
    REPL
        ...
        OpenMV Cams make the REPL available to a host PC over USB (a
        USB CDC serial device).

    small integer
        MicroPython optimises the internal representation of integers such that
        "small" values do not take up space on the :term:`heap`, and
        calculations with them do not require heap allocation. On most 32-bit ports, this
    ...
    webrepl
        A way of connecting to the :term:`REPL` (and transferring files) on a
        device over the internet from a browser. See
