...
device over a serial connection. It works with all OpenMV Cams over their
USB serial connection, and is a command-line alternative to OpenMV IDE for
...
Maintain existing interpreter state for subsequent commands:
...
While the ``repl`` command is running, you can use ``Ctrl-]`` or ``Ctrl-x`` to
exit.

**Note:** The name "REPL" here reflects the common usage of this command
...
Sleep (delay) before executing the next command:
...
This will make the device enter its bootloader. The bootloader is
board-specific — see the **Recovery and debug pins** section of your
board's :doc:`quick reference </openmvcam/quickref>` for details.
...
might be useful to use the ``eval`` command to inspect the state of the
...
Running ``mpremote multiply 3 7`` will set ``x`` and ``y`` as variables on the device, then evaluate the expression ``x*y``.
...
Connect to the device at ``/dev/ttyACM1`` and then run the ``ls`` command.
