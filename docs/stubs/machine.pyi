...
class ADC:
    ...
    them as the id argument to construct an ADC that
    reads one of the on-chip analog sensors. The result of
    read_u16() is the channel’s raw 16-bit reading; for
    calibrated values use the helpers on pyb.ADCAll.
...
class I2C:
    ...
    These methods are only available on the SoftI2C class.
...
class I2CTarget:
    ...
    Each IRQ_* constant is a flag bit. OR them together to
    build a trigger mask for irq(). Inside the handler the
...
class Pin:
    ...
    The constants below are used to configure Pin objects via the
    constructor, init() and irq(). They are grouped by port
    availability.
...
class SoftI2C:
    ...
    are SoftI2C-only – the hardware I2C class does not
    expose them.
