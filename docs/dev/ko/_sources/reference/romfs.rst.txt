...
- **Flexible deployment**: A ROMFS image can be built on a host PC and deployed
  to the device using :ref:`mpremote <mpremote>`, without rebuilding the
  firmware.
...
ROMFS is complementary to both the read-write FAT/LittleFS filesystems (which
...
See :mod:`romfs` for an OpenMV-specific helper that inspects the
...
See the :ref:`mpremote romfs sub-commands <romfs_mpremote_commands>`
...
OpenMV also ships a small :mod:`romfs` helper that prints a formatted
...
.. seealso::
   ...
   :mod:`romfs` -- OpenMV helper for inspecting the mounted ``/rom``
   filesystem.
