...
The OpenMV Cam M7 is a Cortex‑M7 machine‑vision board built around
the STMicroelectronics STM32F765 at 216 MHz with 512 KB of internal
SRAM and 2 MB of internal flash. The bundled OV7725 sensor captures
640×480 grayscale or 320×240 RGB565 frames at up to 150 FPS, and the
10‑pin user header exposes UART, I²C, SPI, CAN, ADC/DAC, and PWM
peripherals.

.. image:: ../openmv-cam-m7-hero.jpg
    :alt: OpenMV Cam M7
    :width: 400px
    :align: center

For full datasheet, photos, and dimensions see the
`OpenMV Cam M7 product page <https://openmv.io/collections/retired-openmv-cams/products/openmv-cam-m7>`_.

Highlights
----------

* **STMicroelectronics STM32F765** Cortex‑M7 at 216 MHz.
* **512 KB internal SRAM** — no external SDRAM.
* **2 MB internal flash** (no external QSPI flash).
* **OV7725** sensor — 640×480 grayscale or 320×240 RGB565 at up to
  150 FPS.
...
.. note::
   ...
   The M7 has **no on‑board power management chip**: there's no
...
.. note::
   ...
   MCU on the M7. Drive or read it externally; you can't toggle it
   from MicroPython.
...
.. warning::
   ...
   You **may** power the M7 by feeding 3.3 V directly into the
...
The M7 has a single user RGB LED plus a pair of high‑power 850 nm
IR LEDs:
...
The OV7725 is driven through the :doc:`/library/omv.csi` module::
...
The sensor is **soldered to the board** on the M7 — it is not on a
swappable module.
...
The ``fir`` driver only talks to the sensor over **I²C 2** — wire
the module to ``P4`` (SCL) and ``P5`` (SDA).
...
The M7 firmware mounts up to three filesystems on boot:
...
* ``/flash`` — **96 KB** FAT filesystem, read/write.
* ``/rom`` — **256 KB** read-only memory-mapped ROMFS.
...
modules — including which ones are unique to the M7 build.
