...
.. image:: ../openmv-h7-plus-hero.jpg
    :alt: OpenMV Cam H7 Plus
    :width: 400px
    :align: center

For full datasheet, photos, and dimensions see the
`OpenMV Cam H7 Plus product page <https://openmv.io/collections/cameras/products/openmv-cam-h7-plus>`_.
...
* **32 MB external SDRAM** (32‑bit @ 100 MHz, 400 MB/s) plus **1 MB
  internal SRAM**.
...
.. note::
   ...
   The H7 Plus has **no on‑board power management chip**: there's no
...
.. note::
   ...
   MCU on the H7 Plus. Drive or read it externally; you can't toggle
   it from MicroPython.
...
A 3.7 V LiPo connector is also present, but **the H7 Plus does not
have a battery charger** — connect a pre‑charged cell, or supply VIN /
USB instead.
...
.. warning::
   ...
   The **battery connector and VIN are tied together** on the H7 Plus.
...
.. warning::
   ...
   You **may** power the H7 Plus by feeding 3.3 V directly into the
...
.. note::
   ...
   The **SWO** trace pin is shared with the camera header's SPI clock
   line. SWO can't be used at the same time as any camera module that
   communicates with the MCU over SPI — for example the **FLIR®
   Lepton® Adapter Module** — pick one or the other.
...
The H7 Plus has a single user RGB LED plus a pair of high‑power 850 nm
IR LEDs:
...
modules — including which ones are unique to the H7 Plus build.
