...
"""
The csi module is the modern, object-oriented interface to
the camera sensor(s) on an OpenMV Cam. Each physical sensor is
represented by a CSI instance, so the multispectral imager
...
constructor. Single-sensor cams just instantiate one CSI.

A CSI object owns the full sensor configuration – pixel
...
auto-exposure, and chip-specific ioctl commands.
Frames are captured with CSI.snapshot(), which returns an
image.Image backed by the frame buffer.

This module supersedes the legacy sensor module (which exposed
the same functionality as module-level functions tied to a single
hidden sensor). New code should use CSI.
...
class CSI:
    ...
    def ioctl(self, request: int, *args) -> Any:
        ...
        IOCTL_* constants; the remaining positional arguments
        ...
        When True, instruct framesize() to optimize for
        ...
        GENX320_BIAS_* constants; value is the
        integer setting.
        ...
        is one of the GENX320_STC_* constants; up to two
        ...
        is one of the GENX320_MODE_* constants. For event
        mode, evt_res is the row-axis length of the event
        buffer passed to IOCTL_GENX320_READ_EVENTS.
        ...
        Returns an image.Image containing the raw event
