...
- It relies on the built-in deflate module, which is present
  on all OpenMV Cams.
...
def compress(data: bytes, wbits: int = 15, /) -> bytes:
    ...
    See the CPython documentation for zlib for more
    ...
    the compressor. See more MicroPython-specific details
...
def decompress(data: bytes, wbits: int = 15, /) -> bytes:
    ...
    The wbits parameter works the same way as for zlib.compress()
    ...
    As for zlib.compress(), see the CPython documentation for zlib
    for more information about the wbits parameter. As for zlib.compress(),
    MicroPython also supports smaller window sizes than CPython. See more
    MicroPython-specific details in the
